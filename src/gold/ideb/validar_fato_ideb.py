...
def validar_estrutura_gold(
    ...
    if list(
        ...
        raise RuntimeError(
            "Esquema da FATO_IDEB diferente do esperado.\n"
            f"Esperado={COLUNAS}\n"
    ...
    if duplicadas.any():
        raise RuntimeError(
            "O grão da FATO_IDEB possui duplicidades."
        )
    ...
    if anos != ANOS_ESPERADOS:
        raise RuntimeError(
            "A Gold não possui exatamente as nove edições "
            "2007–2023 esperadas."
        )
    ...
    if fora.any():
        raise RuntimeError(
            "A Gold contém IDEB fora do domínio 0–10."
        )
...
def validar_integridade_dimensional(
    ...
    if orfas_uf:
        raise RuntimeError(
            f"UFs órfãs na FATO_IDEB: {sorted(orfas_uf)}"
        )

    if orfas_ano:
        raise RuntimeError(
            f"Anos órfãos na FATO_IDEB: {sorted(orfas_ano)}"
        )

    if orfas_etapa:
        raise RuntimeError(
            f"Etapas órfãs na FATO_IDEB: {sorted(orfas_etapa)}"
        )
...
def main():
    print("=" * 110)
    print(
        "VALIDAÇÃO INDEPENDENTE — FATO_IDEB GOLD"
    )
    print("=" * 110)
    print()
