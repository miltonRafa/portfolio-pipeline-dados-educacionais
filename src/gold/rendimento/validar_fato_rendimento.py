...
def validar_estrutura_gold(
    ...
    if list(
        ...
        raise RuntimeError(
            "Esquema da FATO_RENDIMENTO diferente do esperado.\n"
            f"Esperado={COLUNAS}\n"
    ...
    if duplicadas.any():
        raise RuntimeError(
            "O grão da FATO_RENDIMENTO possui duplicidades."
        )
    ...
    if fora.any():
        raise RuntimeError(
            "A Gold contém taxas fora do domínio 0–100."
        )
...
def validar_integridade_dimensional(
    ...
    if orfas_uf:
        raise RuntimeError(
            f"UFs órfãs na FATO_RENDIMENTO: {sorted(orfas_uf)}"
        )

    if orfas_ano:
        raise RuntimeError(
            f"Anos órfãos na FATO_RENDIMENTO: {sorted(orfas_ano)}"
        )

    if orfas_etapa:
        raise RuntimeError(
            f"Etapas órfãs na FATO_RENDIMENTO: {sorted(orfas_etapa)}"
        )
...
def main():
    print("=" * 110)
    print(
        "VALIDAÇÃO INDEPENDENTE — FATO_RENDIMENTO GOLD"
    )
    print("=" * 110)
    print()
    ...
    print(
        "Grão único ANO + UF + ETAPA + REDE + INDICADOR: OK"
    )
    print(
